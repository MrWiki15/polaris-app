...
export const Agenda: React.FC = () => {
  const { data, addEvent, updateEvent, deleteEvent, currentProject } = useApp();
  ...
  const autoAlerts = useMemo(() => {
    ...
    // Overdue debts
    debts
      .filter((d) => !d.paid && d.dueDate && d.dueDate <= today)
      .forEach((debt) => {
        alerts.push({
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              className={cn("font-medium", event.completed && "line-through")}
            >
              {event.title}
            </span>
          </div>
      ...
      {!!currentProject && (
        <div className="mb-4 rounded-xl border border-border p-3 bg-muted/40 text-sm">
          <div className="font-medium">
            Modo proyecto: {currentProject?.name} (Agenda)
          </div>
        </div>
      )}
      {/* Header */}
