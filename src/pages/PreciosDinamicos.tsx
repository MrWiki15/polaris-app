...
import { Tag, TrendingUp, AlertTriangle, Check, Info } from "lucide-react";
import { formatCurrency, calculateOptimalPrice } from "@/lib/storage";
import { cn } from "@/lib/utils";

export const PreciosDinamicos: React.FC = () => {
  const { data, updateProduct, currentProject } = useApp();
  ...
  const avgMargin =
    priceAnalysis.length > 0
      ? priceAnalysis.reduce((sum, p) => sum + p.margin, 0) /
        priceAnalysis.length
      : 0;

  const lowMarginCount = priceAnalysis.filter(
    (p) => p.status !== "good"
  ).length;
      ...
      {!!currentProject && (
        <div className="mb-4 rounded-xl border border-border p-3 bg-muted/40 text-sm">
          <div className="font-medium">
            Modo proyecto: {currentProject?.name} (Precios)
          </div>
        </div>
      )}
      {/* Header */}
      ...
      <div className="bg-info/5 border border-info/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm">
          ...
          <p className="text-muted-foreground">
            Basamos los precios sugeridos en tu costo. Margen conservador (20%),
            óptimo (35%) y agresivo (50%). Un margen saludable está entre 25-40%
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() =>
                      handleApplySuggestion(
                        product.id,
                        product.suggestions.conservative
                      )
                    }
                  ...
                  <button
                    onClick={() =>
                      handleApplySuggestion(
                        product.id,
                        product.suggestions.aggressive
                      )
                    }
                    className="flex items-center gap-2 px-3 py-2 bg-muted hover:bg-muted/80 rounded-lg transition-colors text-sm"
                  >
