...
const tools = [
  ...
  },
  {
    ...
    description: "Controla gastos fijos mensuales",
    color: "text-chart-3",
    bgColor: "bg-chart-3/10",
    link: "/herramientas/pagos-recurrentes",
  },
  {
    icon: Tag,
    ...
    <div className="space-y-6 pb-20">
      {!!currentProject && (
        <div className="mb-4 rounded-xl border border-border p-3 bg-muted/40 text-sm">
          <div className="font-medium">
            Modo proyecto: {currentProject?.name} (Herramientas)
          </div>
        </div>
      )}
      <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl p-4 sm:p-6 border border-primary/20">
        ...
        <p className="text-sm text-muted-foreground">
          Funciones extra para potenciar tu gestión
        </p>
      </div>
          ...
          >
            <div className="flex items-start gap-3 sm:gap-4">
              <div className={cn("p-2.5 sm:p-3 rounded-xl", tool.bgColor)}>
                <tool.icon
                  className={cn("w-5 h-5 sm:w-6 sm:h-6", tool.color)}
                />
              </div>
