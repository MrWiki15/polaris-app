  ...
  const historyData = React.useMemo(() => {
    ...
    return history.map((h, idx) => {
      ...
      if (period === '7d') {
        ...
        label = idx % 5 === 0 ? date.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : '';
      } else {
        label = idx % 15 === 0 ? date.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : '';
      }
      
      return {
        ...h,
        label,
        displayDate: date.toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })
      };
    });
  }, [sales, expenses, period]);

  const totalBalance = historyData.length > 0 ? historyData[historyData.length - 1].cumulative : 0;
  const startBalance = historyData.length > 0 ? historyData[0].cumulative - historyData[0].balance : 0;
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={historyData}>
            ...
            <YAxis 
              stroke="hsl(var(--muted-foreground))" 
              fontSize={11}
              tickFormatter={(value) => formatCurrency(value, settings.currencySymbol)}
            />
            <Tooltip 
              contentStyle={{ 
      ...
      <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-border">
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground">
            {bestDay ? new Date(bestDay.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : '-'}
          </p>
        </div>
        <div className="text-center">
          <p className="text-xs text-muted-foreground mb-1">Peor día</p>
          <p className={cn('font-semibold', (worstDay?.balance || 0) < 0 ? 'text-destructive' : 'text-muted-foreground')}>
            {formatCurrency(worstDay?.balance || 0, settings.currencySymbol)}
          </p>
          <p className="text-xs text-muted-foreground">
            {worstDay ? new Date(worstDay.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : '-'}
          </p>
        </div>
      </div>
    </div>
